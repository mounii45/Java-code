...
public class cycleUndirected {
    ...
    public static boolean dfsUtil(ArrayList<Edge>[] graph, int curr, int par) { // O(V+E)
        ...
        for (int i = 0; i < graph[curr].size(); i++) {// size of neighbors it has connected
            ...
            // case 3 : didnt visit & now visiting
            // par is passed as curr & its neigh as child
            if (!vis[e.dest])// check for the neighbor if it is visited not for curr
            {
                if (dfsUtil(graph, e.dest, curr))// inner if as stack call dont remain
                    return true;
            }

            // case 1 :visited but its not parent
            else if (vis[e.dest] && e.dest != par)
                return true;

            // case 2 -> do nothing continue (if it is visited & parent
            // then there is a chance of thinking its par neigh as cycle )
    ...
    public static boolean dfs(ArrayList<Edge>[] graph) {
        for (int i = 0; i < graph.length; i++) {
            if (!vis[i]) {// for all components check
                // initial par is -1
                if (dfsUtil(graph, i, -1))// helper function for left out
                {
                    return true;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        vis = new boolean[v];
        createGraph(graph);
        System.out.println("cycle is present" + dfs(graph));
    }
}
